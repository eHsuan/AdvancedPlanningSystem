...
namespace AdvancedPlanningSystem.Repositories
{
    public class ApsLocalDbRepository
    {
        ...
        private void SeedTestData(SQLiteConnection conn)
        {
            ...
            using (var transaction = conn.BeginTransaction())
            {
                try
                {
                    ...
                    // 統一參數：batch_size=1, force_idle_sec=18000, max_wait_sec=24000, is_active=1
                    var sb = new System.Text.StringBuilder();
                    sb.Append("INSERT INTO local_config_eqp (eqp_id, max_wip_qty, batch_size, force_idle_sec, max_wait_sec, is_active) VALUES ");

                    // 從提供的清單中提取 (Id, MaxWip)
                    sb.Append("('LF0003', 20, 1, 18000, 24000, 1),"); // 1. ABF壓合
                    sb.Append("('DR0026', 30, 1, 18000, 24000, 1), ('DR0009', 30, 1, 18000, 24000, 1),"); // 2. 雷射
                    sb.Append("('DL0017', 20, 1, 18000, 24000, 1), ('DS0003', 20, 1, 18000, 24000, 1),"); // 3. CF4 Plasma
                    sb.Append("('CL0201', 50, 1, 18000, 24000, 1), ('CL0022', 50, 1, 18000, 24000, 1),"); // 4. 酸洗
                    sb.Append("('CL0200', 50, 1, 18000, 24000, 1),"); // 5. 粗化
                    sb.Append("('SP0002', 15, 1, 18000, 24000, 1), ('SP0004', 15, 1, 18000, 24000, 1),"); // 6. 著膜
                    sb.Append("('LF0014', 20, 1, 18000, 24000, 1),"); // 7. 乾膜壓合
                    sb.Append("('PR0254', 20, 1, 18000, 24000, 1), ('PR0518', 20, 1, 18000, 24000, 1),"); // 8. 綠漆印刷
                    sb.Append("('EX0028', 10, 1, 18000, 24000, 1), ('EX0044', 10, 1, 18000, 24000, 1),"); // 9. 曝光
                    sb.Append("('DE0025', 40, 1, 18000, 24000, 1), ('DE0005', 40, 1, 18000, 24000, 1),"); // 10. 顯影
                    sb.Append("('PL0084', 50, 1, 18000, 24000, 1), ('PL0007', 50, 1, 18000, 24000, 1),"); // 11. 電鍍
                    sb.Append("('ST0012', 40, 1, 18000, 24000, 1),"); // 12. 去膜
                    sb.Append("('ET0023', 40, 1, 18000, 24000, 1),"); // 13. Cu蝕刻
                    sb.Append("('ET0026', 40, 1, 18000, 24000, 1),"); // 14. NiCr蝕刻
                    sb.Append("('VC0680', 10, 1, 18000, 24000, 1),"); // 15. AOI
                    // 16. SMT
                    ...
                    sb.Append("('PL0088', 50, 1, 18000, 24000, 1),"); // 18. NiAu
                    sb.Append("('SM0006', 20, 1, 18000, 24000, 1), ('SM0014', 20, 1, 18000, 24000, 1), ('SM0054', 20, 1, 18000, 24000, 1),"); // 19. uPOL-DB
                    // 20. uPOL-TP
                    ...
                    using (var checkStepCmd = new SQLiteCommand("SELECT COUNT(*) FROM local_config_step_eqp", conn))
                    {
                        if (Convert.ToInt32(checkStepCmd.ExecuteScalar()) == 0)
                        {
                    ...
                    // 3. 初始化 QTime 設定 (local_config_qtime)
                    string qtimeSql = @"
                        INSERT INTO local_config_qtime (from_step_id, to_step_id, transport_buffer_min, qtime_limit_min, updated_at) VALUES 
                        ('STEP_A', 'STEP_B', 15, 120, datetime('now'));
                    ";
                    using (var cmd = new SQLiteCommand(qtimeSql, conn)) { cmd.ExecuteNonQuery(); }
        ...
        private string GetStepEqpMappingSeedSql()
        {
            ...
            // 2. 雷射
            foreach (var step in new[] { "UPLA001", "UPLA006" }) {
                sb.Append($"('{step}', 'DR0026', 1),");
                sb.Append($"('{step}', 'DR0009', 2),");
            }

            // 3. CF4 Plasma
            foreach (var step in new[] { "UPDR003", "UPDR004", "UPDR011" }) {
                sb.Append($"('{step}', 'DL0017', 1),");
                sb.Append($"('{step}', 'DS0003', 2),");
            }

            // 4. 酸洗 / 粗化
            foreach (var step in new[] { "UPCL001", "UPCL012", "UPCL022", "UPCL033", "UPCL003", "UPCL025", "UPCL006", "UPCL007", "UPCL008", "UPCL013", "UPCL028", "UPCL010" }) {
                sb.Append($"('{step}', 'CL0201', 1),");
                sb.Append($"('{step}', 'CL0022', 2),");
            }
            foreach (var step in new[] { "UPBO003", "UPRO005", "UPBO002", "UPRO006", "UPRO003", "UPRO004" }) sb.Append($"('{step}', 'CL0200', 1),");

            // 5. 著膜 (Sputter)
            foreach (var step in new[] { "UPSP006", "UPSP003", "UPSP002", "UPSP005" }) {
                sb.Append($"('{step}', 'SP0002', 1),");
                sb.Append($"('{step}', 'SP0004', 2),");
            }

            // 6. 綠漆印刷
            foreach (var step in new[] { "UPPR005", "UPPR002", "UPPR003", "UPPR004" }) {
                sb.Append($"('{step}', 'PR0254', 1),");
                sb.Append($"('{step}', 'PR0518', 2),");
            }

            // 7. 曝光 (Exposure)
            foreach (var step in new[] { "UPEX029", "UPEX030", "UPEX024", "UPEX023", "UPEX004", "UPEX018", "UPEX019", "UPEX020", "UPEX021", "UPEX027", "UPEX012", "UPEX013", "UPEX014" }) {
                sb.Append($"('{step}', 'EX0028', 1),");
                sb.Append($"('{step}', 'EX0044', 2),");
            }
            ...
            // 9. 電鍍
            foreach (var step in new[] { "UPPL013", "UPPL018", "UPPL010", "UPPL015" }) {
                sb.Append($"('{step}', 'PL0084', 1),");
                sb.Append($"('{step}', 'PL0007', 2),");
            }
            ...
            foreach (var step in new[] { "UPST003", "UPPR004", "UPSC010", "UPST001", "UPRE001", "UPSC011" }) {
                for (int i = 0; i < smtEqps.Length; i++) sb.Append($"('{step}', '{smtEqps[i]}', {i + 1}),");
            }

            // 14. Slicer
            var slicerEqps = new[] { "CU0551", "CU0646", "CU0665", "CU0454", "CU0481", "CU0567", "CU0722" };
            foreach (var step in new[] { "UPSL001", "UPSC012" }) {
                for (int i = 0; i < slicerEqps.Length; i++) sb.Append($"('{step}', '{slicerEqps[i]}', {i + 1}),");
            }

            // 15. DB
            var dbEqps = new[] { "SM0006", "SM0014", "SM0054" };
            foreach (var step in new[] { "UPDB002" }) {
                for (int i = 0; i < dbEqps.Length; i++) sb.Append($"('{step}', '{dbEqps[i]}', {i + 1}),");
            }
            ...
            foreach (var step in new[] { "UPTE001", "UPTE002", "UPQC009", "UPPK002", "UPQC008" }) {
                for (int i = 0; i < tpEqps.Length; i++) sb.Append($"('{step}', '{tpEqps[i]}', {i + 1}),");
            }
        ...
        public List<ConfigQTime> GetQTimeConfigs()
        {
            ...
            using (var conn = new SQLiteConnection(_connectionString))
            {
                ...
                using (var reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        list.Add(new ConfigQTime
                        {
                            FromStepId = reader["from_step_id"].ToString(),
                            ToStepId = reader["to_step_id"].ToString(),
                            TransportBufferMin = Convert.ToInt32(reader["transport_buffer_min"]),
             ...
             using (var conn = new SQLiteConnection(_connectionString))
            {
                ...
                using (var cmd = new SQLiteCommand("SELECT * FROM local_config_eqp WHERE eqp_id = @id", conn))
                {
                    ...
                    using (var reader = cmd.ExecuteReader())
                    {
                        if (reader.Read())
                        {
                            return new ConfigEqp
                            {
                                EqpId = reader["eqp_id"].ToString(),
                                MaxWipQty = Convert.ToInt32(reader["max_wip_qty"]),
                                BatchSize = Convert.ToInt32(reader["batch_size"]),
                                ForceIdleSec = Convert.ToInt32(reader["force_idle_sec"]),
                                MaxWaitSec = Convert.ToInt32(reader["max_wait_sec"]),
        ...
        public List<StatePort> GetActivePorts()
        {
            ...
            using (var conn = new SQLiteConnection(_connectionString))
            {
                conn.Open();
                string sql = @"
                    SELECT p.*, b.carrier_id, b.lot_id, b.dispatch_time 
                ...
                using (var conn = new SQLiteConnection(_connectionString))
                {
                    ...
                    string sql = @"
                        ...
                        (carrier_id, port_id, lot_id, current_step_id, next_step_id, target_eqp_id, 
                         qtime_deadline, dispatch_score, priority_type, is_hold, bind_time, dispatch_time)
                        VALUES (@cid, @pid, @lid, @sid, @nid, @teqp, @dead, @score, @pri, @hold, @btime, @dtime)
                    ";
                    using (var cmd = new SQLiteCommand(sql, conn))
                    {
        ...
        public StateBinding GetBinding(string carrierId)
        {
            using (var conn = new SQLiteConnection(_connectionString))
            {
                ...
                using (var cmd = new SQLiteCommand(sql, conn))
                {
                    ...
                    using (var reader = cmd.ExecuteReader())
                    {
                        if (reader.Read()) return MapBinding(reader);
                    }
                }
            }
            return null;
            ...
            lock (_dbLock)
            {
                using (var conn = new SQLiteConnection(_connectionString))
                {
                    ...
                    using (var transaction = conn.BeginTransaction())
                    {
                        try
                        {
                            string delSql = "DELETE FROM local_state_binding WHERE carrier_id = @cid";
                            using (var cmd = new SQLiteCommand(delSql, conn))
                            {
                                cmd.Parameters.AddWithValue("@cid", transit.CarrierId);
                                cmd.ExecuteNonQuery();
                            }

                            string insSql = @"
                                INSERT OR REPLACE INTO local_state_transit
                                (carrier_id, lot_id, target_eqp_id, next_step_id, dispatch_time, pickup_time, expected_arrival_time, is_overdue)
                                VALUES (@cid, @lot, @teqp, @nstep, @dtime, @ptime, @exp, @ovd)
                            ";
                            using (var cmd = new SQLiteCommand(insSql, conn))
                            {
                                cmd.Parameters.AddWithValue("@cid", transit.CarrierId);
                                cmd.Parameters.AddWithValue("@lot", transit.LotId);
                                cmd.Parameters.AddWithValue("@teqp", transit.TargetEqpId);
                                cmd.Parameters.AddWithValue("@nstep", transit.NextStepId);
                                cmd.Parameters.AddWithValue("@dtime", transit.DispatchTime);
                                cmd.Parameters.AddWithValue("@ptime", transit.PickupTime);
                                cmd.Parameters.AddWithValue("@exp", transit.ExpectedArrivalTime);
                                cmd.Parameters.AddWithValue("@ovd", transit.IsOverdue);
                                cmd.ExecuteNonQuery();
                            }
                            transaction.Commit();
                        }
        ...
        public List<StateTransit> GetAllTransits()
        {
            ...
            {
                conn.Open();
                string sql = "SELECT * FROM local_state_transit";
                using (var cmd = new SQLiteCommand(sql, conn))
                using (var reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        list.Add(MapTransit(reader));
                    }
                }
            }
            return list;
        ...
        public void RemoveTransit(string carrierId)
        {
            lock (_dbLock)
            {
                ...
                {
                    conn.Open();
                    string sql = "DELETE FROM local_state_transit WHERE carrier_id = @cid";
                    using (var cmd = new SQLiteCommand(sql, conn))
                    {
                        cmd.Parameters.AddWithValue("@cid", carrierId);
                        cmd.ExecuteNonQuery();
        ...
        {
            return new StateTransit
            {
                CarrierId = reader["carrier_id"].ToString(),
                LotId = reader["lot_id"].ToString(),
                TargetEqpId = reader["target_eqp_id"].ToString(),
                NextStepId = reader["next_step_id"].ToString(),
                DispatchTime = reader["dispatch_time"].ToString(),
                PickupTime = reader["pickup_time"].ToString(),
                ExpectedArrivalTime = reader["expected_arrival_time"].ToString(),
                IsOverdue = Convert.ToInt32(reader["is_overdue"])
            };
        }
    }
}